...
use crate::model::palette::RGB_INT;
...
impl Canvas {
    pub fn new(image_width: u32, image_height: u32, oversampling: u32, filter_width: u32) -> Self {
        ...
        Self {
            width,
            height,
            data: vec![HDRPixel::zero(); (width * height) as usize],
        }
    }
    ...
    pub fn project_and_update(&mut self, coordinates: &CameraCoordinates, color: &RGB_INT) {
    ...
    // #[inline(never)]
    fn update(&mut self, CanvasPixel(x, y): CanvasPixel, color: &RGB_INT) {
        let pixel_index: usize = (y * self.width + x) as usize;
        self.update_pixel(pixel_index, color);
    }

    // #[inline(never)]
    fn update_pixel(&mut self, index: usize, &RGB_INT(r, g, b): &RGB_INT) {
...
mod canvas_test {
    ...
    fn should_project_coordinates(
        ...
        let canvas = &Canvas::new(width, height, 1, 0);
